...
public class LekarzAmbassador implements FederateAmbassador {
    private LekarzFederate federate;
    ...
    public LekarzAmbassador(LekarzFederate fed){
        this.federate=fed;
    }
    ...
    public void receiveInteraction(InteractionClassHandle interactionClassHandle, ParameterHandleValueMap parameterHandleValueMap, byte[] bytes, OrderType orderType, TransportationTypeHandle transportationTypeHandle, LogicalTime logicalTime, OrderType orderType1, SupplementalReceiveInfo supplementalReceiveInfo) throws FederateInternalError {
        if(interactionClassHandle.equals(wejscieDoLekarzaHandle)){
            StringBuilder builder = new StringBuilder( "Interaction Received:" );
            int id_pacjenta = EncodingHelpers.decodeInt(parameterHandleValueMap.get(LekarzFederate.idPacjentaLekarzHandle));
            LekarzAmbassador.lista.add(id_pacjenta);
            double godzina = EncodingHelpers.decodeDouble(parameterHandleValueMap.get(LekarzFederate.godzinaWejsciaDoLekarzaHandle));
            builder.append("Dodano Pacjenta nr " + id_pacjenta + " do lekarza o godzinie " + godzina);
            log( builder.toString() );
        }
        else if(interactionClassHandle.equals(przeniesieniePacjentaHandle) && EncodingHelpers.decodeInt(parameterHandleValueMap.get(LekarzFederate.miejsceKoncoweHandle)) == 5){
